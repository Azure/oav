...
/*
 * Removes the location pointed by the json pointer in the given doc.
 * @param {object} path The path of the operation. 
...
exports.getProvider = function getProvider(path) {
  ...
  let providerRegEx = new RegExp('\/providers\/(\:?[^{\/]+)', 'i');
  let result;

  let pathMatch = providerRegEx.exec(path);
...
exports.gitClone = function gitClone(url, directory) {
  ...
  if (fs.existsSync(directory) && fs.lstatSync(directory).isDirectory()) {
    throw new Error('directory already exists. Please delete it first and then try again.');
  } else {
