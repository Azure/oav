...
export class LiveValidatorLoader implements Loader<SwaggerSpec> {
  ...
  public resetShouldResolveRef() {
    this.jsonLoader.resetShouldResolveRef();
    this.opts.shouldResolveRef = false;
  }
  ...
  public async load(specFilePath: string): Promise<SwaggerSpec> {
    ...
    if (this.opts.shouldResolveRef) {
      // For fully resolved swagger, we do not apply transformer
      return spec;
    }
