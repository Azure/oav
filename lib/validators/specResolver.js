...
class SpecResolver {
      ...
      if (definitions[modelName].discriminator) {
        ...
        for (let location of locationsToBeUpdated) {
          ...
          if (obj) {
            ...
            obj.oneOf = this.buildOneOfReferences(root);
   ...
   * @returns {PolymorphicTree} root- A PolymorphicTree that represents the model in the inheritance chain.
   */
   ...
   *
   * @param {PolymorphicTree} root- A PolymorphicTree that represents the model in the inheritance chain.
...
/**
 * @class
 * Creates a tree by traversing the definitions where the parent model is the root and child model is one of it's children.
 */
