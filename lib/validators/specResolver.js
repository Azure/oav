  ...
  unifyXmsPaths() {
    ...
    if (xmsPaths && xmsPaths instanceof Object && Object.keys(xmsPaths).length > 0) {
      for (let property in xmsPaths) {
        // Replace the question mark in the paths present in x-ms-paths with forward slash.
        // This avoids messing up the regex validation for url in sway.
        let replacedProperty = property.replace(/\?/ig, "/");
        paths[replacedProperty] = xmsPaths[property];
        delete xmsPaths[property];
      }
