 ...
 */
  resolveDiscriminator() {
    ...
    modelNames.map((modelName) => {
      if (definitions[modelName].discriminator) {
        ...
        let root = subTreeMap.get(modelName), locationsToBeUpdated = [];
        if (!root) {
          root = this.createPolymorphicTree(modelName, definitions[modelName].discriminator, subTreeMap);
        }
        for (let key in references) {
          if (references[key].uri === modelReference && key.indexOf("allOf") === -1) locationsToBeUpdated.push(key);
        }
        for (let location of locationsToBeUpdated) {
          utils.setObject(spec, `${location.slice(1)}`, { "oneOf": this.buildOneOfReferences(root) });
        }
      }
    });
    // delete definitions['Pipeline']['properties']['activities']['items']['$ref'];
    // definitions['Pipeline']['properties']['activities']['items'].oneOf = [
    //   {
    ...
    // delete definitions['ForEachActivityTypeProperties']['properties']['activities']['items']['$ref'];
    // definitions['ForEachActivityTypeProperties']['properties']['activities']['items'].oneOf = [
    //   {
    //     "$ref": "#/definitions/Activity"
    //   },
    //   {
  ...
  createPolymorphicTree(name, discriminator, subTreeMap) {
    ...
    let root = new PolymorphicTree(name);
    let definitions = this.specInJson.definitions;

    // Adding the model name or it's discriminator value as an enum constraint on property marked as discriminator
  ...
  buildOneOfReferences(root) {
    let result = [{ "$ref": `#/definitions/${root.name}` }];
    for (let entry of root.children.entries()) {
      if (entry[1]) {
        result = result.concat(this.buildOneOfReferences(entry[1]));
      }
    }
    return result;
  }
