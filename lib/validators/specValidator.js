    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (location === 'path' || location === 'query') {
        if (location === 'path' && parameterValue && typeof parameterValue.valueOf() === "string") {
          ...
          if (options.pathTemplate.charAt(options.pathTemplate.indexOf(`${parameter.name}`) - 2) === '/' && parameterValue.startsWith('/')) {
            let msg = `In operation "${operation.operationId}", example for parameter "${parameter.name}": "${parameterValue}" starts with a forward slash ` +
              `and the path template: "${options.pathTemplate}" contains a forward slash before the parameter starts. This will cause double forward slashes ` +
