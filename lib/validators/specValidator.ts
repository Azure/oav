...
class SpecValidator {
   ...
   */
  constructErrorObject(code: any, message: string, innerErrors?: null|any[], skipValidityStatusUpdate?: boolean) {
    let err = {
      code: code.name,
      id: code.id,
      message: message,
  ...
  constructRequestResult(operationResult: any, isValid: any, msg: any, requestValidationErrors?: any, requestValidationWarnings?: any) {
    if (!isValid) {
  ...
  constructResponseResult(operationResult: any, responseStatusCode: any, isValid: any, msg: any, responseValidationErrors?: any, responseValidationWarnings?: any) {
  ...
  constructRequestResultWrapper(operationId: any, requestValidationErrors: any, requestValidationWarnings: any, exampleType: any, scenarioName?: any) {
  ...
  constructResponseResultWrapper(operationId: any, responseStatusCode: any, responseValidationErrors: any, responseValidationWarnings: any, exampleType: any, scenarioName?: any) {
    this.initializeExampleResult(operationId, exampleType, scenarioName);
    let operationResult, part, subMsg, infoMsg, errorMsg, warnMsg;
    if (exampleType === Constants.xmsExamples) {
      operationResult = this.specValidationResult.operations[operationId][exampleType].scenarios[scenarioName];
  ...
  validateRequest(operation: any, exampleParameterValues: any) {
    ...
    let result = { request: null, validationResult: { errors: [] as any[], warnings: [] } }, foundIssues = false;
    ...
    if (operation.pathObject && operation.pathObject.api && (operation.pathObject.api.host || hostTemplate)) {
      ...
      if (operation.pathObject.api.schemes && !operation.pathObject.api.schemes.some((item: any) => { return item && item.toLowerCase() === 'https'; }))
  ...
  validateXmsExampleResponses(operation: any, exampleResponseValue: any) {
    ...
    let responseWithoutXmsExamples = utils.getKeys(responsesInSwagger).filter(function (statusCode: any) {
