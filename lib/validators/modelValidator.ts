...
export class ModelValidator extends SpecValidator<SpecValidationResult> {
  ...
  public async validateOperations(operationIds?: string): Promise<void> {
    ...
    if (operationIds) {
      const operationIdsObj: openapiToolsCommon.MutableStringMap<unknown> = {};
      operationIds
        .trim()
        .split(",")
    ...
    for (const operation of operations) {
      ...
      if (openapiToolsCommon.isMapEmpty(openapiToolsCommon.toStringMap(example))) {
  ...
  private initializeExampleResult(
    ...
    if (exampleType === C.exampleInSpec) {
      ...
      if (
        !example ||
        (example && openapiToolsCommon.isMapEmpty(openapiToolsCommon.toStringMap(example)))
      ) {
  ...
  private constructOperationResult(
    ...
    if (exampleType === C.xmsExamples) {
      if (result.scenarios) {
        for (const [scenario, v] of openapiToolsCommon.mapEntries(result.scenarios)) {
          ...
          for (const [responseStatusCode, value] of openapiToolsCommon.mapEntries(
            responseValidation
          )) {
      ...
      if (
        ...
        openapiToolsCommon.toArray(
          openapiToolsCommon.keys(result.requestValidation as openapiToolsCommon.StringMap<unknown>)
        ).length
      ) {
        // requestValidation
      ...
      if (result.responseValidation && !openapiToolsCommon.isMapEmpty(result.responseValidation)) {
        // responseValidation
        for (const [responseStatusCode, value] of openapiToolsCommon.mapEntries(
  ...
  private async validateXmsExamples(operation: Sway.Operation): Promise<void> {
    ...
    if (xmsExamples) {
      for (const [scenario, xmsExampleFunc] of openapiToolsCommon.mapEntries<any>(xmsExamples)) {
  ...
  private validateExampleRequest(operation: Sway.Operation): RequestValidation {
    ...
    if (bodyParam && bodyParam.schema && bodyParam.schema.example) {
      ...
      for (const parameter of parameters) {
        ...
        exampleParameterValues[parameter.name] = parameter.getSample() as any;
      }
  ...
  private validateExampleResponses(
    operation: Sway.Operation
  ): openapiToolsCommon.StringMap<Sway.ValidationResults> {
    if (operation === null || operation === undefined || typeof operation !== "object") {
      throw new Error("operation cannot be null or undefined and must be of type 'object'.");
    }
    const result: openapiToolsCommon.MutableStringMap<Sway.ValidationResults> = {};
  ...
  private validateXmsExampleResponses(
    ...
    const responsesInSwagger: openapiToolsCommon.MutableStringMap<unknown> = {};
    operation.getResponses().forEach((response) => {
      responsesInSwagger[response.statusCode] = response.statusCode;
    });
    for (const exampleResponseStatusCode of openapiToolsCommon.keys(exampleResponseValue)) {
    ...
    const responseWithoutXmsExamples = openapiToolsCommon.toArray(
      openapiToolsCommon.filter(
        openapiToolsCommon.keys(responsesInSwagger),
  ...
  private validateRequest(
    operation: Sway.Operation,
    exampleParameterValues: openapiToolsCommon.StringMap<any>
  ): RequestValidation {
    if (operation === null || operation === undefined || typeof operation !== "object") {
      throw new Error("operation cannot be null or undefined and must be of type 'object'.");
    }
    ...
    let formDataFiles: openapiToolsCommon.MutableStringMap<unknown> | null = null;
    ...
    let request:
      | (msRest.WebResource & { files?: openapiToolsCommon.MutableStringMap<unknown> })
      | null = null;
      ...
      if (!!requestValidationErrors && requestValidationErrors.length > 0) {
        if (exampleFilePath !== undefined) {
          requestValidationErrors.forEach((error) => {
            const positionInfo = openapiToolsCommon.getDescendantFilePosition(
              this.exampleJsonMap.get(exampleFilePath) as openapiToolsCommon.JsonRef,
              error.path
            );
            if (!error.path) {
              error.path = "";
