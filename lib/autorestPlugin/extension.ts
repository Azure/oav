...
export async function openApiValidationExample(
  ...
  try {
    ...
    for (const [op, operation] of mapEntries(specValidationResult.operations)) {
      ...
      for (const [scenario, scenarioItem] of mapEntries(scenarios)) {
        ...
        if (scenarioItem.isValid === false) {
          ...
          // responses
          for (const [responseCode, response] of mapEntries(scenarioItem.responses)) {
