...
class SpecValidator {
  ...
  mergeAllOf(source, target) {
    let self = this;
    //merge the target model's properties
    utils.merge(source.properties, target.properties);
  ...
  getRefDefinitionInAllOf(item) {
    ...
    if (item['$ref']) {
      return self.getDefinitionFromReference(item['$ref']).then(function(result) {
        return Promise.resolve(result);
      }).catch(function(err) {
        return Promise.reject(err);
      });
  ...
  composeAllOf(model, item) {
    ...
    return self.getRefDefinitionInAllOf(item).then(function(result) {
      if (result && result.allOf) {
        self.resolveAllOf(result);
      }
      return Promise.resolve(self.mergeAllOf(model, result));
    }).catch(function(err) {
      return Promise.reject(err);
    });
  }
  ...
  makeModelsStricter() {
    ...
    for (let modelName in definitions) {
      ...
      //every model in the definitions section will have additionalProperties set to false, unless it is explicitly set to true
      if (model && !model.additionalProperties) {
        model.additionalProperties = false;
      }
      //if the model has an allOf then resolve that allOf recursively.
      if (model && model.allOf) {
        self.resolveAllOf(model);
      }
    }
    return;
  }
