...
const HttpTemplate = require('./httpTemplate');
...
class WireFormatGenerator {
  constructor(specPath, specInJson, wireFormatDir) {
    ...
    let wfDir = `${path.dirname(this.specDir)}/wire-format`;
  ...
  processXmsExampleResponses(operation, exampleResponseValue) {
    ...
    if (operation['x-ms-long-running-operation']) {
      result.longrunning = { initialResponse: {}, finalResponse: {} };
    } else {
      result.standard = { finalResponse: {} };
    }
    ...
    for (let exampleResponseStatusCode in exampleResponseValue) {
      ...
      if (response) {
        ...
        if (operation['x-ms-long-running-operation']) {
          if (exampleResponseStatusCode === '202') result.longrunning.initialResponse = exampleResponse;
          if (exampleResponseStatusCode === '200') result.longrunning.finalResponse = exampleResponse;
        } else {
          if (exampleResponseStatusCode === '200') result.standard.finalResponse = exampleResponse;
        }
      }
    }
    return result;
