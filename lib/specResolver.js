...
class SpecResolver {
  ...
  resolve() {
    ...
    return self.unifyXmsPaths()
      .then(() => { return self.resolveRelativePaths(); })
      .then(() => { return self.resolveAllOfInDefinitions(); })
      .then(() => { return self.deleteReferencesToAllOf(); })
      .then(() => { return self.setAdditionalPropertiesFalse(); })
      .then(() => { return self.resolvePureObjects(); })
      .catch(function (err) {
        let e = {
